...
TRM = (function() {
  ...
  TRM.prototype.initial = function(key) {
    return this.key = key;
  };

  TRM.prototype.send = function(path, key, value) {
    var error;
    try {
      return request("" + this.host + path, function(er, res) {
        if (!er) {
  ...
  TRM.prototype._call = function(key, value) {
    return console.log("key and value");
  };
...
all = window || module.exports;

all.analytics = all.analytics || [];

all.analytics = new TRM();

all.analytics.host = "http://localhost:1337";
